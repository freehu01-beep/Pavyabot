...
def run_web():
    ...
    # Use 0.0.0.0 for external access in Replit
    app_web.run(host='0.0.0.0', port=8080)

# Start the Flask server immediately
# This runs the webserver in the background thread
threading.Thread(target=run_web, daemon=True).start()

# 🌸 Secrets
# NOTE: Ensure these are set correctly in Replit Secrets
BOT_USERNAME = os.getenv("BOT_USERNAME", "Pavya_bot")
BOT_TOKEN = os.getenv("BOT_TOKEN")
# Convert to int, safe fallback to 0 if missing
API_ID = int(os.getenv("API_ID", "0") or "0") 
API_HASH = os.getenv("API_HASH", "")
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "")
SELF_URL = os.getenv("SELF_URL", "") # Not used in Replit, but harmless
...
def pavya_ai(prompt: str) -> str:
    ...
    try:
        ...
        # Enforce short reply and length limit
        ...
        return "Hehe... kuch galat ho gaya, thodi der baad try karo na 😅💞"
...
async def help_cmd(_, msg):
    await msg.reply_text(
        ...
        "💡 Group: use @Pavya_bot ya reply to her to talk."
    )
...
async def pavya_chat(_, msg):
    ...
    try:
        ...
        # Generate response (No fixed delay here!)
        reply = pavya_ai(text)

        # Reply immediately once the response is ready
        await msg.reply_text(reply or "Hehe... bolo na kuch aur 😚")
    except Exception as e:
...
async def rob_cmd(_, msg):
    ...
    if is_protected(target):
        await msg.reply_text("🛡️ Iske paas protection hai, tumhara plan fail ho gaya 😜")
        return
...
async def kill_cmd(_, msg):
    ...
    if is_protected(target):
        await msg.reply_text("🛡️ Iske paas shield hai, tumhara tease failed ho gaya 😆")
        return
...
async def revive_cmd(_, msg):
    ...
    if not reviver:
        await msg.reply_text("💳 Pehle /register karo, phir kisi ko revive kar sakte ho 💞")
        return
    ...
    await msg.reply_text(
        f"💫 *Revive Successful!* 🌸\n"
        f"{reviver_u.first_name} ne {target_u.first_name} ko revive kar diya 💖\n"
...
# =========================
# 🌸 SELF-PING (Ignored on Replit)
# =========================
def keep_alive():
    if not SELF_URL:
        logger.warning("SELF_URL not set — self-ping disabled (ok on Replit).")
        return
    while True:
        try:
...
# =========================
# 🌸 MAIN — Works for all Pyrogram versions
# =========================
...
async def main():
    ...
    logger.info("💖 Pavya Baka + Economy Edition is online & ready! (Listening for messages)")
